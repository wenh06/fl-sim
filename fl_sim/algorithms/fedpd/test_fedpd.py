...
from ._fedpd import FedPDClientConfig, FedPDServer, FedPDServerConfig
...
def test_fedpd() -> None:
    ...
    server_config = FedPDServerConfig(10, dataset.DEFAULT_TRAIN_CLIENTS_NUM, p=0.2, stochastic=False, vr=True)
    ...
    server_config = FedPDServerConfig(10, dataset.DEFAULT_TRAIN_CLIENTS_NUM, p=0.2, stochastic=True, vr=True)
    client_config = FedPDClientConfig(dataset.DEFAULT_BATCH_SIZE, 30, vr=True)
    s = FedPDServer(model, dataset, server_config, client_config)
    # s.train_centralized()
    s.train_federated()
