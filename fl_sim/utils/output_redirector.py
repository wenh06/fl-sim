...
class OutputRedirector:
    ...
    def __init__(self, queue: Queue, pid: int, task_tag: str, buffer_size: int = 100, echo_to_original: bool = False):
        """
...
def generate_output_redirector(queue: Queue, task_tag: str) -> OutputRedirector:
    ...
    redirector = OutputRedirector(queue=queue, pid=os.getpid(), task_tag=task_tag)
    ...
    def handle_exception(exc_type: Type[BaseException], exc_value: BaseException, traceback: Optional[TracebackType]):
        if traceback:
            tb_text = "".join(traceback.format_exception(exc_type, exc_value, traceback))
